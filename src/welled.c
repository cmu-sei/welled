...
void signal_handler(void)
{
	...
	print_debug(LOG_DEBUG, "cancelled threads");
}
...
{
	print_debug(LOG_DEBUG, "init_netlink");
	int nlsockfd;
	struct timeval tv;
	...
	if (!sock) {
		print_debug(LOG_ERR, "Error allocating netlink socket");
		running = 0;
		return 0;
	}
...
int recv_from_master(void)
{
	...
	print_debug(LOG_INFO, "received %d bytes in buffer from wmasterd", bytes);

	hdr = (struct message_hdr *)buf;

	print_debug(LOG_DEBUG, "welled version %s, msg size %d", hdr->version, hdr->len);
	...
	while (processed < bytes) {
		...
		if ((bytes - processed) < hdr->len) {
			...
			// TODO store in a buffer somewhere?
			_exit(EXIT_FAILURE);
		}
...
int process_message(char *buf)
{
	...
	/* netlink header */
	nlh = (struct nlmsghdr *)(buf + sizeof(struct message_hdr));
	...
	*/
	...
	if (gnlh->cmd != HWSIM_CMD_FRAME) {
		/* could be multiple messages queued up */
		print_debug(LOG_ERR, "Error - received no frame data in message from wmasterd");
		if (verbose) {
			hex_dump(buf, hdr->len);
		}
		goto out;
	}
...
*/
	...
	if (!devices) {
		print_debug(LOG_INFO, "no devices registered");
		goto out;
	}
	should_ack = 0;
	...
	if (dst == NULL) {
		print_debug(LOG_ERR, "dst device has no address");
		goto out;
	}
	 ...
	 * and this is not an ack packet itself
	 */
	...
	if (any_mac) {
		...
		if (should_ack && !retval) {
			if (verbose)
				printf("- attempting to ack the frame\n");
...
connect:
	...
	do {
		...
		if (ret < 0) {
			...
			if (vsock) {
				print_debug(LOG_NOTICE, "connected to wmasterd on %u:%d",
						servaddr_vm.svm_cid, servaddr_vm.svm_port);
			} else {
				print_debug(LOG_NOTICE, "connected to wmasterd on %s:%d",
						inet_ntoa(servaddr_in.sin_addr), servaddr_in.sin_port);
			}
		}
	} while (running && (ret < 0));
...
{
	print_debug(LOG_DEBUG, "monitor_devices running");
	struct nl_sock *sk;
	struct timeval tv;
	int nlsockfd;
	int opt;
	...
	if (nlsockfd < 0) {
		...
		print_debug(LOG_DEBUG, "monitor_devices setting options for socket %d", nlsockfd);
	}
...
static int list_interface_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb_msg[NL80211_ATTR_IFNAME]) {
		...
		print_debug(LOG_DEBUG, "interface with ifname %s", ifname);
	} else {
		return NL_SKIP;
	}
	...
	if (tb_msg[NL80211_ATTR_IFINDEX]) {
		...
		print_debug(LOG_DEBUG, "interface with ifindex %d", ifindex);
	} else {
		return NL_SKIP;
	}
	...
	if (node == NULL) {
		...
		if (memcmp(&node->address, &addr, ETH_ALEN) != 0) {
			print_debug(LOG_DEBUG, "name changed");
			if (iftype == NL80211_IFTYPE_MONITOR) {
				print_debug(LOG_DEBUG, "monitor mode - setting address to 00's");
				memset(&node->address, 0, ETH_ALEN);
			} else {
				memcpy(&node->address, &addr, ETH_ALEN);
			}
...
{
	print_debug(LOG_DEBUG, "nl80211_get_interface index %d", ifindex);
	struct nl_msg *nlmsg;
	struct nl_cb *cb;
	int err;
	int flags;
...
*/
	...
	if (send_register_msg() == 0) {
		...
		print_debug(LOG_ERR, "could not register with family MAC80211_HWSIM");
		nl_close(sock);
		nl_socket_free(sock);
		nl_cb_put(cb);
		_exit(EXIT_FAILURE);
